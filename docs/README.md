...
2. Read `components/relay/` — per-component reference (added in Phase 3 once the relay ships).
3. Read `runbooks/` — operational procedures (populated as runbooks land).
...
> `owners` handles are quoted. YAML 1.2 §5.3 reserves `@` as a future-use indicator,
> and strict parsers reject unquoted `[@handle]` flow sequences.
