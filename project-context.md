...
The fusion matters. Conflict detection without messaging tells you "your agent conflicts with Riley's branch" — useful. Conflict detection plus consults lets your agent ask Riley's agent _about_ the conflict and resolve it collaboratively — qualitatively different.
...
- _Presence._ Sorted-set in Redis, heartbeat TTL. Diff broadcasts via in-memory pub/sub plus NATS for cross-node fanout.
- _Awareness state._ Live map of `repo → branch → developer → files → intent`. Hot path in Redis with Postgres write-behind. Changes published as deltas.
- _Message router._ `msg.send` → resolve recipients → push if connected, persist to durable inbox if not.
- _Consult coordinator._ Lifecycle state machine, transcript persistence with Ed25519 chaining, multi-party turn ordering (Lamport clocks plus relay-assigned sequence numbers), session multiplexing across a single WS connection.
- _Conflict orchestration._ Receives `conflict.check` requests, fans out to detector workers, returns aggregated results.
- _Durable inbox._ Per-agent message queue (Postgres, S3 archival).
...
Doc lints landed in CI (2026-05-04). All three linters specified in `docs/DOCUMENTATION_PLAN.md` §10.1 are wired and clean against the corpus: markdownlint-cli2 (28 files, 0 errors), vale 3.14 with three custom TAP rules including RFC 2119 vocabulary discipline (43 files, 0 errors / 0 warnings / 0 suggestions), and the new `tools/doclint/doclint.py` (5 of 7 spec'd checks implemented; 2 deferred with inline TODOs). New `doc-lints` CI job + `--dashboard` mode close the doc-dashboard line in `docs/phases/phase-1/exit.md` §7. Side effects on the corpus: ~50 small markdownlint hygiene fixes (language tags, list spacing, bare-email wrapping, etc.) and `owners` front-matter handles quoted across 32 docs (YAML 1.2 reserves `@`). Next: validate the Phase 1 ship criterion with an external implementer, or pick up RFC 0030 (Claude Code adapter), or close the three remaining Phase 1 open questions.

### 9.3 Immediate next steps

**Done in this session (2026-05-04) — doc lints in CI:**

- ✅ `.markdownlint-cli2.yaml` with TAP-tuned rules (relaxed MD013 line-length, MD036 emphasis-as-heading, MD041 first-line-h1, MD060 table-column-style; tightened MD049/MD050 to asterisk; ignores templates and generated trees). Pre-commit hook (`markdownlint-cli2 v0.22.1`) added to `.pre-commit-config.yaml`. Corpus: 0 errors across 28 files.
- ✅ `.vale.ini` with three custom TAP rules at `.vale/styles/TAP/`: `RFC2119.yml` (substitution rule flagging ambiguous phrasings like `MAY NOT` and `CANNOT`), `Keywords.yml` (existence rule flagging ALL-CAPS lookalikes such as `MIGHT`, `OUGHT TO`, `WILL NOT`), `SpecVoice.yml` (existence rule flagging first/second-person voice, scoped to `protocol/SPEC.md` only per `docs/SPEC_STYLE.md` §10). Vocab file at `.vale/styles/config/vocabularies/TAP/accept.txt`. Corpus: 43 files, 0 errors / 0 warnings / 0 suggestions.
- ✅ `tools/doclint/doclint.py` (single-file Python script, PyYAML dep) implementing the five high-value checks from `docs/DOCUMENTATION_PLAN.md` §10.1: front-matter schema validation (`FM001`–`FM011`), 90-day staleness (`FM020`), broken relative markdown links with code-span masking (`LK001`), foundation-tier orphan-doc detection (`OR001`), RFC required-section presence keyed off front-matter status (`RFC001`). Two checks explicitly deferred with inline TODO markers and rationale: undefined glossary terms (needs term-extraction policy) and status-without-decisions-entry (needs git-diff or PR-level integration). `--dashboard` mode emits the markdown dashboard required by §10.4. `--json` mode for tooling integration. `tools/doclint/README.md` documents the check codes, skip rules, and how to add new checks.
- ✅ New `doc-lints` CI job in `.github/workflows/ci.yml` runs all three linters on Ubuntu, downloads vale 3.14.1 directly from errata-ai/vale releases, generates `doc-dashboard.md` and uploads it as a 30-day artifact. Closes `docs/phases/phase-1/exit.md` §7 doc-dashboard line.
- ✅ Corpus normalization: `owners: [@handle]` quoted to `owners: ["@handle"]` across 32 docs (the unquoted form is rejected by strict YAML parsers per YAML 1.2 §5.3 — vale's go-yaml is one of them); ~50 markdownlint hygiene fixes (missing fenced-code language tags converted to `text`, blank lines added around lists per MD032, bare email `security@tap.dev` wrapped as `<security@tap.dev>`, double blank lines collapsed, missing trailing newline on `project-context.md` added, `<reason>` template placeholders backticked to satisfy MD033, table column-count fixed in `THREAT_MODEL.md` by escaping the `||` separator inside the agent_id formula); `docs/README.md` `[components/relay/]` link reworded as plain text (the directory doesn't exist yet — will be linked once Phase 3 ships); schema documentation in `docs/README.md`, `docs/DOCUMENTATION_PLAN.md` §6, and `docs/RFC_PROCESS.md` §3.4 updated to show the quoted `owners` form.

**Done in prior session (2026-04-29) — codegen pipeline pre-flight:**
...
1. **Resolve the remaining Phase 1 open questions** in `docs/phases/phase-1/open-questions.md`: round-trip canonicalization rule (Q-1-001), `tap.dev` provisioning (Q-1-004), and verify the `[verify]` items in `docs/A2A_MAPPING.md` against the live A2A spec (Q-1-005). Q-1-002 and Q-1-006 already closed.
2. **Author the next critical-path Phase 2 RFC: `rfcs/0030-claude-code-adapter.md`.** Consumes RFC 0015 and the daemon RFCs that come next. Defines hook contracts (`SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`), MCP server registration with Claude Code, and the adapter-side conformance tests for the §3.7 sandboxing contract.
3. **Author Phase 1 RFCs `0001-codegen-pipeline.md` and `0002-conformance-runner-contract.md`.** Both describe what shipped on 2026-05-04 and are unchecked in `docs/phases/phase-1/exit.md` §4. Mechanical writeup of the existing implementation while it is still fresh.
4. **Branch protection on `main`.** Configure once permissions allow: required signed commits, two-reviewer approval for `protocol/` and `tools/codegen/`, required CI checks (now including `doc-lints`), no force-push.
5. **Validate Phase 1 ship criterion:** hand the spec + schemas + conformance suite + foundation docs to an external engineer and confirm they can produce a passing client without reading TAP source. Operationalized in `docs/phases/phase-1/exit.md` §1.
...
- **2026-05-04 — Doc lints in CI: markdownlint + vale + custom doclint.** Wired the three doc linters from `docs/DOCUMENTATION_PLAN.md` §10.1 into the project. Tool choices: `markdownlint-cli2` (industry-standard markdown linter, CommonMark-aware, single-config-file format); `vale` 3.x with three custom TAP rules in `.vale/styles/TAP/` (`RFC2119`, `Keywords`, `SpecVoice`) for the spec-style discipline in `docs/SPEC_STYLE.md` §1.3 / §10; `tools/doclint/doclint.py` for project-specific checks (front-matter schema, staleness, broken links, orphan-doc detection, RFC required sections). Two §10.1 checks deferred with inline TODO markers: undefined glossary terms (needs term-extraction policy — Phase 1 follow-up RFC) and status-without-decisions-entry (needs git-diff or PR integration — Phase 2). Legacy-path policy (§11) honored: the six pre-existing files at `docs/{ARCHITECTURE,CODE_OF_CONDUCT,CONTRIBUTING,GOVERNANCE,SECURITY,THREAT_MODEL}.md` get advisory warnings instead of errors until next-touch migration. Vale's strict YAML parser forced `owners: [@handle]` to be quoted as `owners: ["@handle"]` across 32 docs (YAML 1.2 §5.3 reserves `@` as a future-use indicator); schema documentation in `docs/README.md`, `docs/DOCUMENTATION_PLAN.md` §6, and `docs/RFC_PROCESS.md` §3.4 updated to match. New CI job `doc-lints` runs all three linters and uploads `doc-dashboard.md` (generated by `doclint --dashboard` per §10.4) as a 30-day artifact. Closes the doc-dashboard line in `docs/phases/phase-1/exit.md` §7.
